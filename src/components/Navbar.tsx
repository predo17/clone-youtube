...
const Navbar = ({ toggleSidebar, paddingLeft }: NavbarProps) => {
  return (
    <div className="fixed top-0 left-0 w-full h-auto bg-[#0f0f0ff1] backdrop-blur-2xl z-10">
      <header className="h-10 xl:h-16 flex items-center justify-between gap-3 px-4">
        <div className="flex gap-6 items-center">
          <button onClick={toggleSidebar} className="px-2 py-1 hover:bg-zinc-700 rounded-full cursor-pointer hidden xl:block">
            <i className="ri-menu-line text-2xl"></i>
          </button>

          <Link to='/' className="flex items-center gap-3 px-1 relative cursor-pointer">
        ...
        <form className=" xl:flex-1 max-w-2xl ml-4 flex items-center max-xl:gap-4">
          <input
            type="text"
            placeholder="Pesquisar"
            className=" w-full max-w-xl px-6 py-2 bg-zinc-900 rounded-l-full hidden xl:block outline outline-zinc-700 focus:outline-blue-700 z-10"
          />
          ...
          <button className=" md:px-6 md:py-2 xl:bg-zinc-800 xl:border border-zinc-700  rounded-r-full cursor-pointer">
            <i className="ri-search-line"></i>
          </button>
          <button className="px-3.5 py-2.5 bg-zinc-800 rounded-full hover:bg-zinc-700 ml-4 cursor-pointer hidden xl:block">
      ...
      <hr className="min-md:block xl:hidden mb-2 text-zinc-700" />

      <div className={`${paddingLeft}`}>
        {location.pathname === '/' && <CategoryFilter />}{/*para mostra somente na page home*/}
      </div>

      <hr className="block xl:hidden my-2 text-zinc-700" />
    </div>
