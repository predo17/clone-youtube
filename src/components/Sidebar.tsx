...
const Sidebar = ({ isSidebarOpen, isVideoPage = false }: { isSidebarOpen: boolean; isVideoPage?: boolean }) => {
  return (
    <div >
      <aside
        className={`fixed top-16 left-0 h-full w-59 bg-[#0f0f0f] text-white shadow-md py-4 px-2 z-20 max-xl:hidden
         ${isVideoPage ? (isSidebarOpen ? "transition-transform duration-300 ease-in-out translate-x-0 w-20" : " transition-transform duration-200 ease-in-out -translate-x-full") 
